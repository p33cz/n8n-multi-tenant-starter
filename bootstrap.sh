...
# doména je specifická pro tohle nasazení — mimo git (viz .gitignore),
# provisioning skripty (scripts/lib.sh) si ji odtud načtou
cat > "$WORKDIR/domain.env" <<EOF
DOMAIN=${DOMAIN}
EOF
chmod 600 "$WORKDIR/domain.env"
...
- Postgres musí být dosažitelný i ze sítí net-{klient}, ale klient nesmí
  vidět nic dalšího z n8n-mts-net: při zakládání klienta (new-client.sh)
  připoj Postgres kontejner navíc do nově vznikající net-{klient} přes
  \`docker network connect\`, a na maintenance DB i na každé klientské DB
  spusť \`REVOKE CONNECT ON DATABASE ... FROM PUBLIC\`, ať klientův DB
  uživatel vidí jen svou vlastní databázi
...
- HTTP→HTTPS redirect řeš přímo ve vhost šabloně (RewriteCond %{HTTPS}
  off + RewriteRule), NE přes certbot --redirect — při vydávání/obnově
  certifikátu proto používej \`certbot --apache ... --no-redirect\`,
  ať certbot neediktuje vlastní přesměrování do vhostu; pro opětovné
  vydání stejného certifikátu (např. po opravě vhostu) použij navíc
  \`--reinstall\`, ať se nežádá o nový certifikát a nehrozí zbytečné
  narážení na Let's Encrypt rate limit
...
- šablona docker-compose.yml pro PÁR kontejnerů klienta (n8n-{klient} +
  claude-{klient}) ve společné izolované síti net-{klient} — síť
  net-{klient} v šabloně označ jako \`external: true\`, protože ji
  vytváří new-client.sh sám (kvůli připojení Postgresu do ní), ne
  docker compose
...
- DŮLEŽITÉ: /opt/n8n-mts je git repozitář napojený na veřejný GitHub. Doména
  ...
  doménu čti v provisioning skriptech z /opt/n8n-mts/domain.env (já ho
  založím v kroku 6 bootstrapu, mimo git). clients.md a CLAUDE.md se budou
  za provozu plnit reálnými daty klientů — přidej je do .gitignore, ať se
  omylem nedostanou do veřejného repa; do gitu patří jen obecný, znovupoužitelný
  kód (scripts/, templates/, docker/, ops/, pricing.conf)
...
- /opt/n8n-mts/scripts/new-client.sh <jmeno>
  ...
  → vygeneruje n8n API klíč scoped jen na tuto instanci přes n8n REST
    API (/rest/login + /rest/api-keys). Počítej s tím, že:
    - n8n po startu kontejneru potřebuje čas, než jeho API skutečně
      odpovídá — volání obal retry smyčkou (např. 20× po 3s), ne
      jedním pokusem
    - vytvoření klíče vyžaduje povinné pole \`scopes\` — nezapisuj
      seznam scopes natvrdo, stáhni si ho dynamicky z
      /rest/api-keys/scopes, ať to funguje napříč verzemi n8n, a pošli
      k tomu \`expiresAt: null\`
    - skript, co klíč generuje, běží na HOSTITELI (ne v kontejneru) —
      ověř/nainstaluj na hostiteli \`jq\` (a \`curl\`), ať je má čím
      parsovat
    - do logů/výstupu nikdy netiskni samotnou hodnotu klíče ani hesla,
      jen jejich délku, pro ověření že nejsou prázdné
  → spustí v téže síti claude-{jmeno} s tímto klíčem a s
    ANTHROPIC_API_KEY; claude-{jmeno} NESMÍ mít docker.sock mount
  → v claude-{jmeno} automaticky při startu kontejneru (přes entrypoint,
    ...
    POZOR: \`claude remote-control\` funguje jen s přihlášením přes
    claude.ai účet (subscription), NE s ANTHROPIC_API_KEY. Bez
    jednorázového \`claude /login\` zůstane připravený, ale neaktivní.
    Postav to tak, aby to nebránilo zbytku provisioningu (izolace,
    n8n, usage tracking fungují nezávisle na remote-control), a jasně
    mi napiš, že tenhle jeden krok vyžaduje moje ruční přihlášení
...
- vytvoř /opt/n8n-mts/scripts/usage-report.sh, který projde usage.log všech
  ...
  neblokuje). Pokud v usage.log narazíš na model, který není v
  pricing.conf, NESMÍ to shodit celý skript (žádný exit s chybovým
  kódem) — jen vypiš upozornění u toho řádku a pokračuj dál s
  ostatními klienty/modely
...
- zabal sám sebe (ops Claude Code) do Docker kontejneru pojmenovaného
  ...
  stejná poznámka jako u klientů: remote-control potřebuje jednorázové
  \`claude /login\` (subscription účet), s pouhým ANTHROPIC_API_KEY
  zůstane neaktivní; napiš mi to jasně do shrnutí, ať to nepřehlédnu
