...
podle stavu konkrétního serveru. Jsou to obecné šablonové skripty (bez
konkrétní domény, jmen klientů apod.), takže je v pořádku je commitnout
zpět do repa, pokud chceš mít i tuhle část verzovanou.

Naproti tomu `clients.md`, `apps.md` a kořenový `CLAUDE.md` obsahují
živý stav konkrétního nasazení (reálná jména klientů, doménu v odkazech)
a jsou úmyslně v `.gitignore` — do repa nepatří.
...
doporučeno:
- zkontrolovat vygenerované skripty ve `scripts/` (jsou obecné, bez
  konkrétní domény/jmen klientů) a commitnout je
